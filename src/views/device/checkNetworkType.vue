<template>
  <main>
    ...
    <p>调用此方法会通知 App 检查当前接入网络连接的类型，主要用于识别是 4g / 5g 或者 wifi。</p>
    <h5>{{ "<template />" }}</h5>
...
const xmlcode: string = `<button @click="checkNetworkType">检查网络连接类型</button>`;

const jscode: string = `const checkNetworkType = () => {
  fc.checkNetworkType();
};`;

const jscode2: string = `// 等待网络类型检查结果
